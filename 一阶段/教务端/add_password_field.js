...
db.connect((err) => {
    ...
    // 给students表添加password字段
    const addFieldSql = 'ALTER TABLE students ADD COLUMN password VARCHAR(255) AFTER email';
    
    db.query(addFieldSql, (err, results) => {
        if (err) {
            console.error('❌ 添加password字段失败:', err);
            return;
        }
        console.log('✅ password字段已添加到students表');
